...
public class CityMap {
    ...
    private void makeGraph() throws IOException {
        ...
        List<Graph.Node<City>> nodes = graph.getNodes();
        for (int i = 0; i < nodes.size() - 2; i++) { // BUG: -2 instead of -1
            Graph.Node<City> node1 = nodes.get(i);
            for (int j = i + 1; j < nodes.size(); j++) {
                Graph.Node<City> node2 = nodes.get(j);
                double distance = distanceInMiles(node1.getData(), node2.getData());
                graph.addEdge(node1, node2, (int) distance);
            }
        }
    }
}
